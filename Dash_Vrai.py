...
# Pour le prétraitement avec PySpark
...
def preprocess_data(df):
    ...
    # Remplissage des valeurs manquantes avec la modalité la plus fréquente
    mode_status = df1.groupby("status").count().orderBy("count", ascending=False).first()[0]
    df1 = df1.fillna(mode_status, subset=["status"])
    mode_category = df1.groupby("category_name_1").count().orderBy("count", ascending=False).first()[0]
    df1 = df1.fillna(mode_category, subset=["category_name_1"])
    # Suppression des lignes manquantes sur certaines colonnes
    df1 = df1.na.drop(subset=["Working Date", "sku", "Customer ID"])
    # Conversion des dates
    df1 = df1.withColumn("order_date", F.to_date(F.unix_timestamp("created_at", "M/d/y").cast("timestamp")))
...
# Initialisation de la session Spark et chargement/prétraitement des données
spark = get_spark_session()
df = load_data(spark)
df1 = preprocess_data(df)

# Conversion du DataFrame Spark en Pandas pour Dash (attention aux gros volumes)
pdf = df1.toPandas()
